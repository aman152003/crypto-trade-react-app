...
function CryptoList() {
    ...
    const fetchCryptoData = async() => {
        try{
            const result = await fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=250&sparkline=false&price_change_percentage=24h');
    ...
    if(cryptoData.length === 0 || cryptoData === undefined || cryptoData === null){
        return <Loading />
    }else{
        return (
            <div className="home">
                ...
                <div style={searchTerm ? {display:'none'} : {display:'flex'}} className="pagination-btns">
                    <button onClick={prevPage}><img src={leftArrowIcon} /></button>
                    <button style={{marginLeft:'0.5rem'}} onClick={nextPage}><img src={rightArrowIcon} /></button>
                </div>
            </div>
        )
    }
